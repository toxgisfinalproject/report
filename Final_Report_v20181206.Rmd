...
census_income_df = readRDS("census_income_pop_tidy.rds") %>% 
  janitor::clean_names() %>% 
...
tobacco_2010_present = read_csv(file = "./data_smoke/BRFD_tobacco_2010_present.csv") %>% 
...
tobacco_2010_prior = read_csv(file = "./data_smoke/BRFD_tobacco_2010_prior.csv") %>% 
  filter(Response == "Never") %>% 
  dplyr::select(year = "YEAR", st = "LocationAbbr", response = "Response", percent_never = "Data_Value") %>%
  janitor::clean_names() %>% 
  mutate(percent_smoke = as.numeric(100 - percent_never),
